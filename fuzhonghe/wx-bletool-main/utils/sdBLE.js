...
class sdBLE {
  ...
  startConnect(deviceId, deviceName = "未知设备") {
    if (this.globalData.bluetoothState) {
      wx.createBLEConnection({
        ...
        success: (res) => {
          console.log("连接结果111111111111：", res)
          // wx.navigateTo({
          //   url: `/pages/service/service?deviceId=${deviceId}&deviceName=${deviceName}`,
          // });
          // 触发事件
          this.globalData.emit('connectedDevice', {
            deviceId: deviceId,
  ...
  onBLECharacteristicValueChange(callback) {
    wx.onBLECharacteristicValueChange((res) => {
      ...
      let receiverValue = new Float32Array(res.value)
   
      
      callback(receiverValue);
    });
  }
  /**
   * BLE蓝牙特征值写数据：向蓝牙发送数据
